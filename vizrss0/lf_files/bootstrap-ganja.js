...
(function ($) {
	...
	Powwow.init = function (options) {
		...
		switch (type) {
		case 'usercomments':
			Powwow.loadUsercomments(options);
			break;
		case 'userreplies':
			Powwow.loadUserreplies(options);
			break;
		case 'forum':
	...
	/**
	 * Powwow.loadUsercomments loads comments for the selected SSO id
	 * @param {Object} [params] parameters passed to api request
	 * @param {String} [params.username] the SSO username of user passed to Powwow to load comments
	 * @public
	 */
	Powwow.loadUsercomments = function (options) {
		var params = options.params;
		var config = options.config;
		// user name
		comment_data.username = params.username || null;
		// load user comments
		comment_data.mode = 'usercomments';
		comment_data.domain = config.domain;
		comment_data.host = config.host;
	...
	/**
	 * Powwow.loadUserreplies loads comment replies for the selected SSO id
	 * @param {Object} [params] parameters passed to api request
	 * @param {String} [params.username] the SSO username of user passed to Powwow to load comments
	 * @public
	 */
	Powwow.loadUserreplies = function (options) {
		var params = options.params;
		var config = options.config;
		// user name
		comment_data.username = params.username || null;
		// load user replies
		comment_data.mode = 'userreplies';
		comment_data.domain = config.domain;
		comment_data.host = config.host;
	...
	function getIframeSrc() {
		...
		return getIframe().data('src') + 'conversation.html' + (suffix.length ? '?' + suffix.join('&') : '') + '#' + encodeURIComponent(window.location.href);
	}
	...
	function onMessage(e) {
		...
		case 'powwowLoaded':
			...
			if (deferred_messages.length > 0) {
				for (var i in deferred_messages) {
				...
				if (message_data.id !== undefined) {
					Powwow.getCommentOffset(message_data.id, function (offset) {
						if (offset) {
							...
							jQuery(document).stop().scrollTo(top, 125);
						}
					})
				}
			}, 1000);
		...
		case 'setUrl':
			if (typeof history.pushState !== 'undefined' && message_data.url !== undefined) {
				...
				var url = (message_data.url == '') ?  window.location.pathname : message_data.url;
				var leftover = jQuery.map(window.location.search.substring(1).split('&'), function (e) { return e.indexOf('comment=') !== -1 ? null: e; }).join('&');
